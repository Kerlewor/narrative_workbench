...
- **v0.3.0 上下文引擎：** `relevance_resolver.py`（精确上下文注入，替代 context_builder 核心逻辑）、`ledger_manager.py`（7 类结构化 JSONL 账本 CRUD）、`render_views.py`（账本→Markdown 可读视图渲染）。
- **v0.3.0 章节统筹：** `director_sheet.py`（章节导演表生成与验证）、场景接力卡 + 全章连续性审查模板。
- **v0.3.0 平台原生化：** `sync_skills.py`（skills/ → .claude/skills/ + .agents/skills/ 同步）、`AGENTS.md`（Codex 原生入口）、`.codex/`（Agent + hooks）、CLAUDE.md 精简至 128 行核心路由。
