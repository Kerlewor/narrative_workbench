...
```text
$ cd narrative_workbench && claude                  # 进入模板目录，启动 Claude Code
```

```text
你: 这是一个 AI 小说项目。请读取 CLAUDE.md。

AI: 好的。你是否安装了 Python 3？是否使用辅助脚本？

你: 已安装。

AI: [加载完成] 当前是模板目录。要创建新项目还是直接开始？
      如果创建新项目，告诉我书名即可。

你: 创建项目"临安雪"

AI: [运行 create_project.py] 项目已创建在临安雪/。
      要开始搭建大纲吗？
...
如果没有 Python 环境，AI 会跳过脚本，改用手动等效检查。已有大纲时，将 `搭建大纲` 替换为 `导入现成大纲`。
