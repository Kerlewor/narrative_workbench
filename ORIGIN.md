...
InkOS 的核心工作流模式——多 Agent 写作流水线、真相文件状态管理、伏笔生命周期追踪、canonical/working 双区分离、去 AI 味机制——通过分析 InkOS 生成的小说项目结构而习得。Narrative Workbench 将这些模式独立重新构想为面向 Claude Code 与 Codex CLI 的 Markdown-native prompt 工程框架。

Narrative Workbench **不是 InkOS 的移植、fork 或重新实现**。它是在不同平台上以根本不同的架构构建的独立框架。开发过程中未使用或参考任何 InkOS 源码（TypeScript）。
