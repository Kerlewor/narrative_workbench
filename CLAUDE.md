...
3. 调用 Review Agent 进行一致性审查。**Review 不直接改文**，只输出问题清单：
   ...
   - 是否忘记推进关键伏笔（对照半衰期到期的活跃 hook）；
   - 是否提前泄露秘密（对照 intent/plan 中的"暂不掀"和知识库 locked 事实）；
   - 是否和前文状态冲突（对照 current_state）；
   - 是否节奏失衡、存在 AI 味或说明腔、场景目标不清。
4. 问题清单按严重度排列。每条标注位置、问题和修改方向建议。是否采纳由作者决定。

### 润色第 N 章（共创模式）

对作者手写章节按指定模式润色。触发语包括：
...
1. 确认手写稿路径和润色模式（默认 `preserve-author-style`）。
2. 调用 Polish Agent 按指定模式润色。**默认不覆盖原稿**——输出为独立润色稿，标注所有改动位置和改动原因。
3. 作者审核后选择采纳、修改或拒绝每处改动。

### 第 N 章写作简报

在作者动笔前，生成该章的写作约束简报。触发语包括：
...
1. 读取卷纲确认本章在整卷中的位置和功能。
2. 读取当前活跃 hook 和半衰期状态，列出本章必须推进的伏笔。
3. 读取角色卡和当前状态，列出本章角色行为边界。
4. 读取 intent/plan（如已规划），列出禁止提前泄露的信息。
5. 生成简报，包含：本章类型、必须处理、禁止事项、推荐写法。格式简洁，一屏内读完。
