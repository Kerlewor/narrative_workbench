...
3. **不得绕过门禁。** gatekeeper 通过 + final-check 完成后，才能由主会话写入 `chapters/`。
...
story/outline/    大纲、分卷、题材手册（canonical）
story/roles/      角色卡（canonical）
story/ledger/     结构化事实账本（JSONL，程序检索）
story/views/      作者可读视图（Markdown，脚本生成）
story/plans/      章节导演表（YAML）
story/runtime/    临时任务包、审阅结果、候选稿（working 区）
workflow/         系统原则与协议文档
scripts/          确定性 Python 脚本（25 个）
skills/           可插拔 Skill（正式来源）
.claude/skills/   平台原生 Skills 入口（同步脚本生成）
...
每次新会话:
1. **先检查 Python 环境** — 有 Python 则使用脚本，无 Python 则手动等效执行
...
# Polish: context_builder.py --chapter N --agent polish
# Review: context_builder.py --chapter N --agent review; 若有候选正文: text_audit.py
...
# 批末审计:   doctor.py + chapter_index.py --check + hook_report.py + hook_matrix.py + structure_report.py
# 进入新卷前: structure_report.py + hook_matrix.py --current N + doctor.py

# === 维护 ===
# 修改 skill 后: skill_check.py + doctor.py
# 手动改状态后: doctor.py (必要时 structure_report.py)
# 更新伏笔看板: render_views.py all
# 同步 Skills:   sync_skills.py
```
